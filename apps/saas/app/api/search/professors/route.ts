...
async function handleSearch(keywords?: string[]) {
  ...
  const t0 = Date.now()
  const kws = Array.isArray(keywords) && keywords.length > 0 ? keywords : [
  ...
  async function searchOne(q: string): Promise<Item[]> {
    ...
    if (tvly) {
      try {
        const r = await fetch('https://api.tavily.com/search', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${tvly}` }, body: JSON.stringify({ query: q, search_depth: 'advanced', max_results: 5, include_answer: false }) })
    ...
    if (brave) {
      try {
        const r = await fetch(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(q)}&country=BR&lang=pt-BR&count=5`, { headers: { 'X-Subscription-Token': brave } })
        const j = await r.json().catch(() => null)
    ...
    if (fire) {
      try {
        const r = await fetch('https://api.firecrawl.dev/v1/search', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${fire}` }, body: JSON.stringify({ query: q, limit: 5 }) })
        const j = await r.json().catch(() => null)
        for (const it of Array.isArray(j?.results) ? j.results : []) {
          const title = typeof it?.title === 'string' ? it.title : ''
          const uri = typeof it?.url === 'string' ? it.url : ''
  ...
  function score(it: Item): number {
    ...
    if (h.includes('youtube.com') || h.includes('instagram.com')) { if (t.includes('br') || t.includes('brasil')) s *= 1.6 }
    return s
  }
  function nameFrom(title: string): string { const parts = title.split(' - '); return parts[0].trim() }
  const collected: Item[] = []
  for (const q of kws) { const items = await searchOne(q); collected.push(...items) }
  const seen = new Set<string>()
  const ranked: Ranked[] = []
  for (const it of collected) { if (seen.has(it.uri)) continue; seen.add(it.uri); const sc = score(it); ranked.push({ name: nameFrom(it.title), uri: it.uri, score: sc }) }
...
export async function POST(req: Request) {
  ...
  const keywords = Array.isArray(body?.keywords) && body.keywords.length > 0 ? body.keywords : undefined
  const res = await handleSearch(keywords)
  return res
  } catch {
