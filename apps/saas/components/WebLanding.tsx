...
const WebLanding: React.FC<LandingPageProps> = ({ onStartTrial, onLogin }) => {
  return (
      ...
      <section className="relative pt-40 pb-20 lg:pt-48 lg:pb-32 z-10">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div className="text-center lg:text-left space-y-8 order-2 lg:order-1">
            ...
            <div className="pt-8 flex items-center justify-center lg:justify-start gap-4 text-xs font-mono text-slate-500 uppercase tracking-widest"><span>Midea</span> • <span>Samsung</span> • <span>LG</span> • <span>Gree</span> • <span>Daikin</span></div>
          </div>
      ...
      <section className="py-24 bg-[#0f1115] relative z-10 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-slate-400 text-lg">Substitua 10 grupos e 5 manuais em PDF por uma única conversa.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 grid-rows-2 gap-4 h-auto md:h-[600px]">
             <div className="md:col-span-2 md:row-span-2 bg-[#18181b] rounded-3xl p-8 border border-white/5 relative overflow-hidden group hover:border-emerald-500/30 transition-colors">
                 ...
                 <div className="relative z-10 h-full flex flex-col">
                     ...
                     <p className="text-slate-400 mb-8 leading-relaxed">Não é um bot de respostas prontas. É uma IA que raciocina sobre o problema. Ele entende &quot;barulho estranho&quot;, analisa fotos de placas queimadas e sugere testes elétricos complexos.</p>
             ...
             <div className="md:col-span-1 md:row-span-1 bg-[#18181b] rounded-3xl p-6 border border-white/5 relative overflow-hidden group hover:border-blue-500/30 transition-colors">
                 ...
                 <p className="text-xs text-slate-400">Mande áudio no meio da escada. Receba áudio enquanto dirige.</p>
             </div>
             <div className="md:col-span-1 md:row-span-1 bg-[#18181b] rounded-3xl p-6 border border-white/5 relative overflow-hidden group hover:border-purple-500/30 transition-colors">
                 ...
                 <p className="text-xs text-slate-400">Foto da etiqueta? Ele lê o modelo. Foto do erro? Ele diagnostica.</p>
             </div>
             <div className="md:col-span-2 md:row-span-1 bg-gradient-to-r from-emerald-900/40 to-[#18181b] rounded-3xl p-6 border border-white/5 flex items-center justify-between relative overflow-hidden"><div className="relative z-10"><h3 className="text-xl font-bold text-white mb-2">Manuais Ilimitados</h3><p className="text-sm text-slate-400 max-w-xs">Acesse a biblioteca técnica completa de Midea, LG, Samsung e Carrier instantaneamente.</p></div><div className="text-6xl opacity-20 grayscale saturate-0 animate-pulse">📚</div></div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-4xl mx-auto px-6 relative z-10">
               <div className="bg-gradient-to-b from-[#1c1c21] to-[#131316] rounded-[40px] p-1 border border-white/10 shadow-2xl">
                   <div className="bg-[#18181b] rounded-[36px] p-8 md:p-16 text-center relative overflow-hidden">
                       ...
                       <p className="text-slate-400 mb-10">Sem pegadinhas. Cancele quando quiser.</p>
                       <div className="flex items-center justify-center gap-2 mb-10"><span className="text-6xl font-bold text-white tracking-tighter">{PLAN_PRICE}</span><div className="text-left text-sm text-slate-500 leading-none"><div>/mês</div><div>fatura mensal</div></div></div>
                       ...
                       <button onClick={onLogin} className="w-full md:w-auto bg-white text-slate-950 px-12 py-4 rounded-xl font-bold text-lg hover:bg-emerald-400 transition-all shadow-lg hover:shadow-emerald-500/20">Assinar Agora</button>
                       {process.env.NODE_ENV !== 'production' && (
                         <a
                           href="https://checkout.stripe.com/test_dev_placeholder"
                           className="mt-4 inline-block bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-emerald-500 transition-colors"
                           aria-label="Assinar com Stripe (Dev)"
                         >
                           Checkout Stripe (Dev)
                         </a>
                       )}
                       <p className="mt-4 text-xs text-slate-600">7 dias de garantia incondicional.</p>
                   </div>
               </div>
          </div>
      </section>
      <footer className="py-12 border-t border-white/5 bg-[#0a0a0c] text-center text-slate-500 text-sm"><p className="mb-2">Feito para o campo 🇧🇷</p><p>&copy; {new Date().getFullYear()} ZapPRO AI. Powered by OpenAI GPT-4o.</p></footer>
    </div>
  )
}
